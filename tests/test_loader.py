...
def rest_rules():
    ...
    competencies = get_competencies(root)
    return get_weekly_rest_rules(root), get_daily_rest_rules(root), get_employees(root, competencies)


def test_loading_competencies(comptencies):
    assert len(comptencies) == 3, "Expected 3 competencies got %d" %(len(comptencies)) 
...
def test_days_with_demand(days_with_demand):
    for day in range(4):
        assert days_with_demand[day].demand_id == "DayDemandId1", "Expect the correct demand definition for the first four days"
    for day in range(4,len(days_with_demand)):
        assert days_with_demand[day].demand_id == "DayDemandId2", "Expect the correct demand definition for the last three days"
