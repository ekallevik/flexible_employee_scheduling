...
@pytest.mark.parametrize("working_days, consecutive_day_limit, expected", [
    ([0, 0, 0, 0, 0, 0, 0, 0], 3, 0),
    ([1, 1, 0, 1, 1, 0, 1, 1], 3, 0),
    ([1, 1, 1, 0, 1, 0, 1, 1], 3, 1),
    ([1, 1, 1, 0, 0, 1, 1, 1], 3, 2),
...
def test_count_consecutive_days(validator, working_days, consecutive_day_limit, expected):
    ...
    assert validator.count_consecutive_day_violations(working_days, consecutive_day_limit) ==expected
...
@pytest.mark.parametrize("working_days, consecutive_day_limit", [
    ([1, 1, 1, 1, 0, 1], 5),
    ([1, 1, 1, 1], 5)
])
def test_violates_consecutive_days_raises_error_on_wrong_input(validator, working_days, consecutive_day_limit):

    with pytest.raises(ValueError):
        validator.violates_consecutive_days(working_days, consecutive_day_limit)


@pytest.mark.parametrize("working_days, expected", [
...
@pytest.mark.parametrize("working_days", [
    ([1, 1, 1, 1]),
    ([1, 1])
])
...
@pytest.mark.parametrize("working_days, saturdays, expected", [
    ([1, 1, 1, 1, 1, 1], [5], False),
