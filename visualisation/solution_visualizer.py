...
class Visualizer():
    ...
    #Paths
        ...
        #Demand needed for problem
        self.demand = get_days_with_demand2(root)
    
        # creating indices
        self.key_y = re.compile('\d*,\d*,\d*')
        self.key_x = re.compile('\d*,\d+\.?\d*,\d*\.?\d*')
        self.key_w = re.compile('\d*,\d*.?\d*,\d*.?\d*')

        self.maximum = max([self.demand[day].ideal[i] for day in self.demand for i in range(len(self.demand[day].start))])
        self.minimum = min([self.demand[day].ideal[i] for day in self.demand for i in range(len(self.demand[day].start))])

        #Todays date for timeline
        self.today = datetime.combine(date.today(), dt(0,0,0))
        self.solution = str(solution_folder / (problem_name + "_solution.sol"))


    def visualize_demand(self, min_demand, ideal_demand, max_demand, number_of_days):
        """
    ...
    def create_gantt_chart(self):
        #days = periods * 7
        df = []
        for day in self.demand:
            for i in range(len(self.demand[day].start)):
                demand_start_time = datetime.combine(day, self.demand[day].start[i])
                demand_finish_time = datetime.combine(day, self.demand[day].end[i])
                dictionary = dict(Task="Demand",
                            Start=demand_start_time,
                            Finish=demand_finish_time,
                            Resource="Demand_" + str(self.demand[day].ideal[i]))
        ...
        with open(self.solution) as file:
            line = file.readline()
            ...
            while line:
                if not (line.rstrip().endswith('0')):
                    ...
                    if line.startswith('x'):
                        k = self.key_x.findall(line)[0].split(',')
                        if(float(k[1]) - int(float(k[1])) != 0):
                            minute = 60/(1/(float(k[1]) - int(float(k[1]))))
                        else:
                            minute = 0
                        start_time = self.today + timedelta(hours=int(float(k[1])), minutes=minute)
                        ...
                        finish_time = self.today + timedelta(hours=int(end), minutes= minute)
                    ...
                    if line.startswith('w'):
                        k = self.key_w.findall(line)[0].split(',')
        ...
        # print(colors)
        red = Color("#8ff9ff")
        c = list(red.range_to(Color("#146266"),(self.maximum - self.minimum)+2))
        colors2 = {"Demand_" + str(i): c[i-1].hex for i in range(self.minimum, self.maximum+1)}
