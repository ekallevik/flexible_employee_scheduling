...
def delta_calculate_deviation_from_demand(
    ...
    for c in competencies:
        for e2, t, v in destroy_repair_set:
            for t in t_covered_by_shift[t, v]:
                state.soft_vars["negative_deviation_from_demand"][c, t] = max(
                    0,
                    demand["ideal"][c, t]
...
# Weaknesses:
# 1. It checks every possible combination of isolated working days instead of the ones that could be
# affected by the destroy and repair
...
# Weaknesses:
# 1. It checks every possible combination of isolated off days instead of the ones that could be
# affected by the destroy and repair
...
# Weaknesses:
# 1. It checks every possible combination of consecutive days instead of the ones that could be
# affected by the destroy and repair
            ...
            state.soft_vars["consecutive_days"][e, i] = max(
                ...
                (
                    sum(
                        ...
                        for i_marked in range(i, i + L_C_D + 1)
                    )
                )
                - L_C_D,
            )
...
def calculate_f(
    state, employees, saturdays, days, L_C_D, weeks, weights, preferences, competencies
):

    for e in employees:
        state.f[e] = calculate_f_for_employee(
            L_C_D, days, e, saturdays, state, weeks, weights, preferences, competencies
        )


def calculate_f_for_employee(
    L_C_D, days, e, saturdays, state, weeks, weights, preferences, competencies
):
    ...
    f = (
        ...
        * sum(state.soft_vars["consecutive_days"][e, i] for i in range(len(days) - L_C_D))
        + weights["preferences"]
        * sum(preferences[e][t] * state.y[c, e, t] for t in preferences[e] for c in competencies)
    )
    ...
    for e, t1, v1 in repair_set:
        for t in t_covered_by_shift[t1, v1]:
            state.hard_vars["cover_multiple_demand_periods"][e, t] = max(
                0, (sum(state.y[c, e, t] for c in competencies if state.y.get((c, e, t))) - 1)
            )
        ...
        for t in t_covered_by_shift[t1, v1]:
            state.hard_vars["mapping_shift_to_demand"][e, t] = max(
                ...
                abs(
                    ...
                    - sum(state.y[c, e, t] for c in competencies if (c, e, t) in state.y)
                ),
            )
...
def calculate_daily_rest_error(state, destroy_and_repair, invalid_shifts, shift_combinations_violating_daily_rest, shift_sequences_violating_daily_rest):
    ...
    for e, t, v in repair:
        ...
        if (t, v) in shift_sequences_violating_daily_rest[e]:
            value = max(
                0,
                sum(state.x[e, t2, v2] for t2, v2 in shift_sequences_violating_daily_rest[e][t, v])
                - 1,
            )
...
def hard_constraint_penalties(state):
    ...
    logger.info(
        f"Violations: [d=({below_demand}, {above_demand}), x={break_one_shift_per_day}, "
...
def calculate_objective_function(
    state, employees, saturdays, L_C_D, days, weeks, weights, preferences, competencies
):
    ...
    for j in weeks:
        ...
        if setting == "worst":
            value[j] = (
                ...
                - sum(
                    10 * abs(state.soft_vars["deviation_from_ideal_demand"][c, t])
                    for c in competencies
                    for t in time_periods_in_week[c, j]
                )
                - sum(8 * state.soft_vars["partial_weekends"][e, (5 + j * 7)] for e in employees)
                - sum(
                    10 * state.soft_vars["isolated_working_days"][e, i + 1]
                ...
                - 10
                * sum(
                    state.hard_vars["below_minimum_demand"].get((c, t), 0)
                ...
                - 10
                * sum(
                    state.hard_vars["more_than_one_shift_per_day"].get((e, i), 0)
                    for e in employees
                    for i in days_in_week
                )
                - 10
                * sum(
                    state.hard_vars["cover_multiple_demand_periods"].get((e, t), 0)
                    for e in employees
                    for j in weeks
                ...
                - max(
                    0,
                    sum(
                        2 * state.soft_vars["deviation_contracted_hours"].get((e, j), 0)
                        for e in employees
                    ),
                )
                - 10
                * sum(
                    state.hard_vars["delta_positive_contracted_hours"].get(e, 0) for e in employees
                )
                - 10
                * sum(
                    state.hard_vars["daily_rest_error"].get((e, i), 0)
                    for e in employees
                    for i in days_in_week
                )
            )
            ...
            value[j] = (
                ...
                * max(
                ...
                - 10
                * sum(state.hard_vars["weekly_off_shift_error"].get((e, j), 0) for e in employees)
                - 100
                * sum(
                    state.hard_vars["delta_positive_contracted_hours"].get(e, 0) for e in employees
                )
...
def regret_objective_function(
    ...
    return (
        ...
        * sum(
        ...
        - 10
        * sum(state.hard_vars["more_than_one_shift_per_day"].get((employee, i), 0) for i in days)
        - 10
        * sum(
            state.hard_vars["cover_multiple_demand_periods"].get((employee, t), 0)
            for t in t_changed
        )
        - 10 * sum(state.hard_vars["weekly_off_shift_error"].get((employee, j), 0) for j in weeks)
        - 10
        * sum(state.hard_vars["mapping_shift_to_demand"].get((employee, t), 0) for t in t_changed)
        - 10 * state.hard_vars["delta_positive_contracted_hours"].get(employee, 0)
        - 100 * competency_score
    )


def employee_shift_value(
    state,
    e,
    shift,
    saturdays,
    ...
    week = int(day / 7)
    daily_rest_error = regret_daily_rest_error(
        state,
        day,
        e,
        shift,
...
def regret_weekly_rest(state, shifts_at_week, e, week, shift):
    actual_shifts = [
        (t, v)
        for t, v in shifts_at_week[week]
        if state.x[e, t, v] == 1 or (t, v) == (shift[0], shift[1])
    ]
    off_shift_periods = []
    ...
    if not actual_shifts:
        ...
        if week_interval[1] - (actual_shifts[-1][0] + actual_shifts[-1][1]) >= 36:
            off_shift_periods.append(
                week_interval[1] - (actual_shifts[-1][0] + actual_shifts[-1][1])
            )

        for i in range(len(actual_shifts) - 1):
            if actual_shifts[i + 1][0] - (actual_shifts[i][0] + actual_shifts[i][1]) >= 36:
                off_shift_periods.append(
                    actual_shifts[i + 1][0] - (actual_shifts[i][0] + actual_shifts[i][1])
                )
...
def f_regret_values(state, e, shift, invalid_shifts, shift_combinations_violating_daily_rest, shift_sequences_violating_daily_rest, weeks, saturdays, sundays, days, shifts_at_week, shifts_at_day, weights, preferences, t_covered_by_shift, L_C_D):
    ...
    total_negative_deviation_from_contracted_hours = (
        sum(state.soft_vars["deviation_contracted_hours"][e, j_2] for j_2 in weeks) - shift[1]
    )
    ...
    current_week_rest = regret_weekly_rest(state, shifts_at_week, e, week, shift)
    current_isolated_days = regret_isolated_days(state, e, shifts_at_day, day, weeks)
    current_partial_weekends = regret_partial_weekend(
        state, e, shifts_at_day, saturdays, sundays, day
    )
    consecutive_days = regret_consecutive_days(state, e, shift, weeks, shifts_at_day, L_C_D, day)
