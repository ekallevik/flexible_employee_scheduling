    ...
    for c in competencies:
        for e2,t,v in destroy_repair_set:
            for t in t_covered_by_shift[t,v]:
                state.delta[c,t] = abs(sum(state.y[c,e,t] for e in employee_with_competencies[c]) - demand["ideal"][c,t])


def delta_calculate_negative_deviation_from_contracted_hours(state, repair_set, destroy_set):
    for e,t,v in destroy_set:
        state.delta_negative_contracted_hours[e] -= v
...
def calculate_partial_weekends(state, repair_destroy_set, shifts_at_day, saturdays):
    for e,t1,v1 in repair_destroy_set:
        for i in saturdays:
            state.partial_weekend[e,i] = abs(sum(state.x[e,t,v] for t,v in shifts_at_day[i]) - sum(state.x[e,t,v] for t,v in shifts_at_day[i+1]))
...
def calculate_isolated_working_days(state, repair_destroy_set, shifts_at_day, days):
    for e,t1,v1 in repair_destroy_set:
        for i in range(len(days)-2):
            state.isolated_working_days[e,i+1] = max(0,(-sum(state.x[e,t,v] for t,v in shifts_at_day[i]) 
...
def calculate_isolated_off_days(state, repair_destroy_set, shifts_at_day, days):
    for e,t1,v1 in repair_destroy_set:
        for i in range(len(days)-2):
            state.isolated_off_days[e,i+1] = max(0,(sum(state.x[e,t,v] for t,v in shifts_at_day[i]) 
            - sum(state.x[e,t,v] for t,v in shifts_at_day[i+1]) 
...
def calculate_consecutive_days(state, repair_destroy_set, shifts_at_day, L_C_D, days):
    for e,t1,v1 in repair_destroy_set:
        for i in range(len(days)-L_C_D):
            state.consecutive_days[e,i] = max(0,(
                sum(state.x[e,t,v] for t,v in shifts_at_day[i_marked] for i_marked in range(i,i+L_C_D)))- L_C_D)
...
def calculate_f(state, repair_destroy_set, off_shifts, saturdays, days, L_C_D):
    for e,t1,v1 in repair_destroy_set:
        state.f[e] = (sum(v * state.w[e,t,v] for t,v in off_shifts)
            - state.delta_c[e]
            - sum(state.partial_weekend[e,i] for i in saturdays)
            - sum(state.q_iso_work[e,i+1] for i in range(len(days)-2))
            - sum(state.q_iso_off[e,i+1] for i in range(len(days)-2))
            - sum(state.q_con[e,i] for i in range(len(days)-L_C_D)))
    ...
    for c in competencies:
        for e2,t1,v1 in repair_destroy_set:
            for t in t_covered_by_shift[t1,v1]:
                state.below_minimum_demand[c,t] = max(0, (demand["min"][c,t] - sum(state.y[c,e,t] for e in employee_with_competencies[c])))
...
def under_maximum_demand(state, repair_destroy_set, employee_with_competencies, demand, time_periods, competencies, t_covered_by_shift):
    for c in competencies:
        for e2,t1,v1 in repair_destroy_set:
            for t in t_covered_by_shift[t1,v1]:
                state.above_maximum_demand[c,t] = max(0, (sum(state.y[c,e,t] for e in employee_with_competencies[c]) - demand["max"][c,t]))
    
def maximum_one_shift_per_day(state, repair_destroy_set, demand, shifts_at_day, days):
    for e,t,v in repair_destroy_set:
        for i in days:
            state.more_than_one_shift_per_day[e,i] = max(0, (sum(state.x[e,t,v] for t,v in shifts_at_day[i]) - 1))

def cover_only_one_demand_per_time_period(state, repair_set, t_covered_by_shift, competencies):
    for e,t1,v1 in repair_set:
        for t in t_covered_by_shift[t1,v1]:
            state.cover_multiple_demand_periods[e,t] = max(0,(sum(state.y[c,e,t] for c in competencies) - 1))

def one_weekly_off_shift(state, repair_destroy_set, weeks, off_shift_in_week):
    for e,t,v in repair_destroy_set:
        for j in weeks:
            state.weekly_off_shift_error[e,j] = max(0,(abs(sum(state.w[e,t,v] for t,v in off_shift_in_week[j]) - 1)))

def no_work_during_off_shift(state, repair_destroy_set, competencies, t_covred_by_shift, off_shifts):
    for e,t2,v2 in repair_destroy_set:
        for t1,v1 in off_shifts: 
            if state.w[e,t1,v1] == 1:
                state.no_work_during_off_shift[e,t1] = sum(state.y[c,e,t] for c in competencies for t in t_covred_by_shift(t1,v1))


def mapping_shift_to_demand(state, repair_destroy_set, t_covered_by_shift, shifts_overlapping_t, competencies):
    for e,t1,v1 in repair_destroy_set:
        for t in t_covered_by_shift[t1,v1]:
           state.mapping_shift_to_demand[e,t] = max(0, abs(sum(state.x[e, t_marked, v] for t_marked, v in shifts_overlapping_t[t]) - sum(state.y[c,e,t] for c in competencies)))


def calculate_positive_deviation_from_contracted_hours(state, destroy_set, repair_set):
    for e,t,v in destroy_set:
        state.delta_positive_contracted_hours[e] -= v
...
def calculate_objective_function(state, repair_destroy_set, off_shifts, saturdays, L_C_D, days, competencies):
    ...
    objective = (sum(state.f.values()) + g - sum(state.delta.values()))
    return objective
