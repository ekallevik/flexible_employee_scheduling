def calculate_deviation_from_demand(model):
    ...
    for c in model.competencies:
        for t in model.time_periods:
            delta[c, t] = abs(
                sum(model.y[c, e, t] for e in model.employee_with_competencies[c])
...
def calculate_negative_deviation_from_contracted_hours(model):
    ...
    for e in model.employees:
        delta_negative_contracted_hours[e] = len(model.weeks) * model.contracted_hours[e] - sum(
            model.time_step * model.y[c, e, t]
            for t in model.time_periods
            for c in model.competencies
        )
...
def calculate_partial_weekends(model):
    ...
    for i in model.saturdays:
        for e in model.employees:
            if (
                abs(
                    sum(model.x[e, t, v] for t, v in model.shifts_at_day[i])
                    - sum(model.x[e, t, v] for t, v in model.shifts_at_day[i + 1])
                )
                != 0
            ):
                partial_weekend_shifts.extend(
                    [(e, t, v) for t, v in model.shifts_at_day[i] if model.x[e, t, v] == 1]
                )
                partial_weekend_shifts.extend(
                    [(e, t, v) for t, v in model.shifts_at_day[i + 1] if model.x[e, t, v] == 1]
                )
            ...
            consecutive_days[e, i] = max(
                ...
                (
                    sum(
                        ...
                        for i_marked in range(i, i + model.limit_on_consecutive_days)
                    )
                )
                - model.limit_on_consecutive_days,
            )
    ...
    for e in model.employees:
        for i in model.days:
            more_than_one_shift_per_day[e, i] = max(
                0, (sum(model.x[e, t, v] for t, v in model.shifts_at_day[i]) - 1)
            )
    ...
    for e in model.employees:
        for t in model.time_periods:
            cover_multiple_demand_periods[e, t] = max(
                0, (sum(model.y[c, e, t] for c in model.competencies) - 1)
            )
    ...
    for e in model.employees:
        for j in model.weeks:
            weekly_off_shift_error[e, j] = max(
                0, (abs(sum(model.w[e, t, v] for t, v in model.off_shift_in_week[j]) - 1))
            )
    ...
    for e in model.employees:
        for t in model.time_periods:
            mapping_shift_to_demand[e, t] = max(
                0,
                abs(
                    sum(model.x[e, t_marked, v] for t_marked, v in model.shifts_overlapping_t[t])
                    - sum(model.y[c, e, t] for c in model.competencies)
                ),
            )
...
def calculate_f(model, employees=None):
    ...
    for e in employees:
        f[e] = (
            sum(v * model.w[e, t, v] for t, v in model.off_shifts)
            - delta_c[e]
...
def hard_constraint_penalties(model):
    ...
    hard_penalties = (
        ...
        + break_weekly_off
        + break_no_work_during_off_shift
...
def calculate_objective_function(model):
    ...
    objective = (
        sum(f[e] for e in model.employees)
        + g
